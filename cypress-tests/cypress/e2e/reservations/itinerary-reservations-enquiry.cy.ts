...
const calculateTotalRate = (availability: IProtelAvailability) => {
  return availability.rates_data.reduce(
    (acc, rate) => acc + Number(rate.room_rate),
    0
  );
};
...
describe("itinerary-reservations-enquiry", () => {
  ...
  beforeEach(() => {
    ...
    cy.intercept("POST", new RegExp(/api\/v1\/availabilities/)).as(
      "getAvalabilitiesApi"
    );
    cy.login();
  });
  ...
  const expectReservationCardData = (availability: IProtelAvailability) => {
    ...
    cy.getByCy("arrival_date").should(
      "contain",
      dateFormatter.dddotmmdotyyyy(arrivalDate)
    );
    cy.getByCy("departure_date").should(
      "contain",
      dateFormatter.dddotmmdotyyyy(departureDate)
    );
    cy.getByCy("total").should(
      "contain",
      priceFormatter.formatPrice(calculateTotalRate(availability))
    );
  };
